...
/**
 * Async sample: create a custom analyzer using the create or replace API.
 *
 * Prerequisites:
 ...
 *
 * Run:
 *   pnpm ts-node src/contentAnalyzersCreateOrReplace.ts
 */
...
const main = async (): Promise<void> => {
  ...
  const analyzerId = `sdk-sample-custom-analyzer-${Math.floor(Date.now() / 1000)}`;
  ...
  if (["202", 202, "200", 200, "201", 201].includes(initialResponse.status)) {
    try {
      ...
      console.log("Analyzer details:", JSON.stringify(result, null, 2));

      // 6. Demo: Clean up the created analyzer
