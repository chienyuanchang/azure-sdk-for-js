 ...
 *   These variables can be set in a .env file in the typescript directory for repeated use. Please see sample.env for an example.
 *
 * Run:
 *   npx ts-node contentAnalyzersAnalyzeUrl.ts
 */
 ...
 * @param result - The AnalyzeResult from the API
 */
const printAnalysisResult = (result: any): void => {
  ...
  if (content.kind === "document") {
    ...
    if (doc.tables?.length) {
      ...
      doc.tables.forEach((table: any) => {
        ...
        const colCount: number = table.columnCount;
  ...
  // To get the full raw JSON response, see the sample: contentAnalyzersAnalyzeBinaryRawJson.ts
  // import { saveJsonToFile } from "./sampleHelper.ts";
...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    const result = (pollResult.body as any)?.result ?? pollResult.body;
